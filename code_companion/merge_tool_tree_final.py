# # AI_tools/code_companion/merge_tool_tree_final.py
# import os
# import json
# import fnmatch
...
from core.prompt_builder import PromptBuilder
from core.git_engine import get_git_diff, get_changed_files
...
class CodeCompanionApp:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("Code Companion")
    ...
    def _build_ui(self):
        ...
        tk.Button(top, text="Select Root Directory",
                  command=self.select_root).pack(side="left", padx=5, pady=5)
        ...
        tk.Button(top, text="Generate AI Prompt",
                  command=self.generate_ai_prompt).pack(side="right", padx=5)
    ...
    def select_root(self):
        ...
        self.insert_tree_nodes("", self.root_dir)
    ...
    def insert_tree_nodes(self, parent, path):
        for item in os.listdir(path):
            ...
            if os.path.isdir(full_path):
                self.insert_tree_nodes(node, full_path)
    ...
    def generate_ai_prompt(self):
        ...
        if self.changed_only_var.get():
            changed_files = get_changed_files(self.root_dir)
            merged_code = FileEngine.merge_changed_files_only(
                self.root_dir,
                changed_files
            )
        else:
            ...
            merged_code = FileEngine.merge_files_with_relative_paths(
                self.selected_files,
                self.root_dir
            )
        ...
        self.show_prompt_window(prompt, stats)

    # ============================================================
    # Prompt Window
    # ============================================================

    def show_prompt_window(self, prompt, stats):
        ...
        # ===== Stats Bar =====
        stats_frame = tk.Frame(win, bg="#f0f0f0")

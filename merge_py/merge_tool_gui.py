...
def get_all_py_files(paths):
    ...
    py_files = []
    for path in paths:
        path = path.strip("{}")  # Handle drag paths with curly braces
...
class MergeToolApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Python Merge Tool (Drag & Drop)")
    ...
    def drop_files(self, event):
        """Handle files dragged into Listbox."""
        ...
        self.refresh_file_list_display()
        if added_count > 0:
            messagebox.showinfo("Info", f"Added {added_count} Python files via drag & drop.")
...
if __name__ == "__main__":
    root = TkinterDnD.Tk()  # Use TkinterDnD root
